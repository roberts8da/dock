...
WSPATH = os.environ.get('WSPATH', UUID[:8] if UUID else 'ws')
...
class ProxyHandler:
    def __init__(self, uuid: str):
        self.uuid = uuid
        self.uuid_bytes = bytes.fromhex(uuid) if uuid else b''
...
async def websocket_handler(request):
    ...
    CUUID = UUID.replace('-', '') if UUID else ''
    path = request.path
            ...
            async with session.get(url, timeout=60) as resp:
                if resp.status == 200:
                    ...
                    with open('npm', 'wb') as f:
                        f.write(content)
                    os.chmod('npm', 0o755)
                    return os.path.isfile('npm') and os.access('npm', os.X_OK)
    except:
        pass
    return False
...
def is_npm_running():
    try:
        ...
        return './npm' in out or '/npm ' in out or out.find(' npm') >= 0
    except:
        return False
...
async def run_nezha():
    ...
    if not ok or not os.path.isfile('npm'):
        return
    ...
    if NEZHA_SERVER and NEZHA_PORT and NEZHA_KEY:
        ...
        command = (
            f'nohup ./npm -s {NEZHA_SERVER}:{NEZHA_PORT} -p {NEZHA_KEY} {nezha_tls} '
            f'--disable-auto-update --report-delay 4 --skip-conn --skip-procs '
            f'>/dev/null 2>&1 &'
        )
...
def cleanup_xray_files():
    files = [
        'config.json', 'xray', 'xray.zip', 'xray.pid',
        'sub.txt', 'list.txt', 'subscribe.txt',
        'v2ray', 'v2ray.zip', 'sing-box', 'sb', 'sb.zip',
        'geoip.dat', 'geosite.dat', 'LICENSE', 'README.md'
    ]
    for file in files:
        try:
...
async def main():
    # 强制使用平台分配的 PORT，禁止自动换端口（否则反代会 Connection refused）
    actual_port = PORT
    ...
    print(f"server is running on port {actual_port}", flush=True)

    asyncio.create_task(run_nezha())

    async def delayed_cleanup():
        await asyncio.sleep(300)   # 5 分钟后清 xray 残留
        cleanup_xray_files()
        await asyncio.sleep(120)   # 再等 2 分钟清哪吒文件
        cleanup_nezha_files()
